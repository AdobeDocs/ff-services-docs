--- 
title: About Using Asynchronous APIs
description: Understand asynchronous APIs and how they are used with Firefly service APIs.
---

# Using asynchronous APIs

Asynchronous APIs allow you to make a request and recieve the response asynchronously. Learn what to expect from asynchronous calls, and review our example implementations so you can manage your Firefly requests and responses more efficiently.

## Overview

Some of the original Firefly APIs operate in a synchronous fashion. That means when you call the API with your request, to generate an image for example, the Firefly platform delays returning the response until it finishes generating your assets.

An asynchronous API allows a client to make a request, then the server processes the request in the background, and the client can continue with other tasks until the result is ready. Firefly's asynchronous APIs provide a rapid response to your request that includes a job ID and URLs that can be used to check the status of your request or to cancel the job.

## Asynchronous API workflows

Any workflow with Firefly's asynchronous APIs should include these steps:

1. Make a request with any arguments and parameters.
2. Firefly replies with a rapid response, returning a job ID and URLs that can be used to check the status of your request or to cancel the job.
...
3. Check on the job with scheduled requests. For example, check once every second and wait for a success or failure message.
4. On success, Firefly sends a result with the output URLs.
5. Retrieve your assets from the output URL.

Some of the Firefly's asynchronous APIs include:
...
It may help to see the power of asynchronous APIs by studying them in action. Review these example implementations to understand more about how to use them.

###  Making an asynchronous request

Here is a complete script that takes a static text prompt, creates a request to [Firefly's Generate Image Async API](../api/image_generation/V3_Async/), and checks the status of this request. When Firefly generates the image, this script saves the result to the file system.

<InlineAlert variant="[info]" slots="header, text" />

INFO

Any utility functions, such as the one handling authentication, are no different from synchronous calls, and your code may implement this authentication differently.
