...
First, you will see this in action by building a simple demo that shows the result of *not* specifying a class, followed by specifying each of the two options and note the differences.

Below is the generic function which will be used to try out the image generation features with a prompt and optional content class:
...
Now, use the following code to run three tests - one without specifying the `contentClass` and one each for each class:
...
Once again, you can use a simple utility method to test this:
...
The results of using the presets specified above (`golden` and `pastel_color`) are below:
