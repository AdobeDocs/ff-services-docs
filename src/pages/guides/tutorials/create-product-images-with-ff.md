...
This method is passed two main arguments (ignoring the credentials) - `text` and `imageId`, representing our prompt and reference image. You can see in `data` where these values are passed in. Finally, this is passed to the Firefly [Generate Images API](../../firefly-api/guides/api/generate-images/V3/) API endpoint. The result, in this case the URL of the image, is returned. 
...
This method wraps the [Generative Expand API](../../firefly-api/guides/api/expand-image/V3/). It needs both the image resource to expand (which we got from the initial Generate Image API prompt) and the desired size. In this case, we need a link to the result so the URL is returned.
