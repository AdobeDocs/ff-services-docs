...
Within your shell, set the the `$CLIENT_ID` and `$ACCESS_TOKEN` environment variables to be able to run the below curl commands successfully. For example, run `export CLIENT_ID=your-client-id`.

### Step 1: Create an asset

Before you can run speech enhancement on an `Asset`, you'll need to register a new `Asset` with the Enhance Speech API. To accomplish this, use the `POST /v2/assets` endpoint.
...
The `byteSize` and `mediaType` are used to enable secure and verifiable direct uploads of your asset to our object store.
...
In order to create `SpeechEnhancement`s for the `Asset`, the `Asset`'s `status` must be `ready_for_processing`. To do this, you need to upload valid data to the object store using the `uploadUrl` included in the response to `POST /v2/assets`.

You can accomplish this by making a `PUT` request to the `uploadUrl` with the `Asset`'s content and setting the `Content-Type` header to match the `mediaType` provided when creating the `Asset`:
...
If the upload request succeeds, you'll receive a `200` response with an empty body. If you receive a `400`-level response, double check that you are providing the exact same `Content-Type` and `Content-MD5` as supplied when creating the `Asset` via `POST /v2/assets`.
...
#### `uploadUrl` expiration window

The presigned `uploadUrl` is *only valid for 10 minutes*. Be certain to `PUT` to `uploadUrl` *as soon as it is returned in the API* (without any systematic delays).

To receive a new `uploadUrl` for an `Asset`, call `GET /v2/assets/:id` and use the new `uploadUrl` in the response:

```bash
curl --request GET 'https://audio-services-stage.adobe.io/api/v2/assets/3445d7a0-2513-4606-b37b-bff0034c0214' \
--header "x-api-key: $CLIENT_ID"  \
--header "Content-Type: application/json" \
--header "Authorization: Bearer $ACCESS_TOKEN"
```
...
```json
{
    ...
    "status": "waiting_for_upload",
    "createdAt": "2024-04-09T22:37:12.541Z",
    "updatedAt": "2024-04-09T22:37:12.541Z",
    "byteSize": 59400,
    "mediaType": "audio/mpeg",
    "checksum": null
}
...
As an API client, the best way to ensure that not only did the `PUT` request above go through successfully, but was successfully registered by the [Enhance Speech API](./api/get_enhancement.md), is to call the `GET /v2/assets/:id` endpoint and ensure that `status` shows `ready_for_processing`. Once the `Asset` is `ready_for_processing`, an `uploadUrl` will no longer be displayed:
...
```json
{
    ...
    "createdAt": "2024-04-09T21:38:36.872Z",
    "updatedAt": "2024-04-09T22:37:03.129Z",
    "byteSize": 59400,
    "mediaType": "audio/mpeg",
...
Once you have an `Asset` that is `ready_for_processing`, you can create a `SpeechEnhancement` for it, at a given strength, using the `POST /v2/speech-enhancements` endpoint:

```bash
curl 'https://audio-services-stage.adobe.io/api/v2/speech-enhancements' \
--header "x-api-key: $CLIENT_ID" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer $ACCESS_TOKEN" \
--data '{
    "input": {
        "uploadId": "audio-services:asset:3445d7a0-2513-4606-b37b-bff0034c0214"
    },
    "strength": 93
}'
```
...
```json
{
    ...
    "output": null,
    "assetId": "3445d7a0-2513-4606-b37b-bff0034c0214",
    "requestedProcessingAt": "2024-04-09T22:56:50.174Z",
...
For example, if you create 5 assets via `POST /v2/assets`, upload data for all of them, and then create 5 speech enhancements via `POST /v2/speech-enhancements`, only 1 of those speech enhancements will be processed at any given time (the order of processing will be from oldest to newest).

To see the latest status of the newly created `SpeechEnhancement`, you can poll `GET /v2/speech-enhancements/:speech-enhancement-id`:

```bash
curl 'https://audio-services-stage.adobe.io/api/v2/speech-enhancements/b877bc56-9c17-4efb-8eec-b5839cd7f555' \
--header "x-api-key: $CLIENT_ID" \
--header "Authorization: Bearer $ACCESS_TOKEN"
```
...
To download the `SpeechEnhancement`, make a `GET` request to the `download_url` displayed above. This is a (short-lived) presigned URL that will enable the downloading of the enhanced audio from AWS S3. If the presigned URL has expired, make a new request to `GET /v2/assets/:asset-id/speech-enhancements/:speech-enhancement-id` to get a new presigned URL.
