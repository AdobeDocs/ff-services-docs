- pathPrefix:
    - /firefly-services/docs/

- pages:
    - [All Firefly Services](/guides/index.md)
    - [Firefly API](https://developer.adobe.com/firefly-services/docs/firefly-api/?aio_internal)
    - [Creative Production API](https://developer.adobe.com/firefly-services/docs/workflow-builder-api/?aio_internal)
    - [Photoshop API](https://developer.adobe.com/firefly-services/docs/photoshop/?aio_internal)
    - [Lightroom API](https://developer.adobe.com/firefly-services/docs/lightroom/?aio_internal)
    - [Audio/Video API](https://developer.adobe.com/audio-video-firefly-services/?aio_internal)
    - [InDesign API](https://developer.adobe.com/firefly-services/docs/indesign-apis/?aio_internal)
    - [Substance 3D API](https://developer.adobe.com/firefly-services/docs/s3dapi/?aio_internal)
    - [Illustrator API](https://developer.adobe.com/firefly-services/docs/illustrator/?aio_internal)
    - [Content Tagging API](https://experienceleague.adobe.com/docs/experience-platform/intelligent-services/content-commerce-ai/overview.html)

- buttons:
    - [Support](https://airtable.com/appu5RTWgdM95jynx/pagyuT1qspNJcPU2E/form)
    - [Console](https://developer.adobe.com/console/) consoleId
    
- subPages:
    - [Overview](/guides/index.md)
    - [Getting credentials](/guides/get-started.md)
    - [Support](/guides/support/index.md)
    - [Firefly tutorials](/guides/tutorials/index.md)
        - [Creating Product Images at Scale](/guides/tutorials/create-product-images-with-ff.md)
        - [Automate a Content Workflow](/guides/tutorials/automate-workflow.md)
        - [Getting Started with the SDK](/guides/tutorials/using-the-sdk.md)
