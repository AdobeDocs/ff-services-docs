---
name: api-fix
description: Fixes only issues reported by Redocly lint on an OpenAPI JSON spec. Use when the user invokes /api-fix or asks to fix API lint or Redocly errors. Does not apply style guide or copy fixes.
---
...
Do **not** apply FFS style guide changes, copy edits, license boilerplate, section reorder, anyOf cleanup, or api-eval findings unless Redocly also reports them.
...
5. If lint is clean but style/copy issues remain, say: *Lint clean. Run `/api-eval` for style guide and documentation findings.*
